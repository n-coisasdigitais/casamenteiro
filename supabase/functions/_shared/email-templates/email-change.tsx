  ...
  <Html lang="pt-BR" dir="ltr">
    ...
    <Body style={main}>
      <Container style={container}>
        ...
        <Text style={text}>Você solicitou a alteração do seu e-mail no {siteName} de <Link href={`mailto:${oldEmail}`} style={link}>{oldEmail}</Link> para <Link href={`mailto:${newEmail}`} style={link}>{newEmail}</Link>.</Text>
        <Button style={button} href={confirmationUrl}>Confirmar alteração</Button>
        <Text style={footer}>Se você não fez esta solicitação, proteja sua conta imediatamente alterando sua senha.</Text>
      </Container>
    </Body>
  </Html>
)
...
const button = { backgroundColor: '#D9905A', color: '#ffffff !important' as any, border: '1px solid #C97D46', fontSize: '15px', fontWeight: '600' as const, borderRadius: '999px', padding: '14px 28px', textDecoration: 'none', display: 'inline-block' }
