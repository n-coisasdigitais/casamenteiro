...
Deno.serve(async (req) => {
  ...
  if (tipo === "assinatura" && referenciaId) {
    const { data: assinatura } = await admin
      ...
      .maybeSingle();
    if (!assinatura) {
      await registrar({ resultado: "assinatura_nao_encontrada" }, 200);
      return json({ ok: true, ignored: "assinatura_nao_encontrada" });
    }
    if (aprovado) {
      // REGRA B: se o fornecedor ainda está em trial, a vigência (e a próxima cobrança)
      // começa no FIM do trial — ele assinou cedo mas só "conta" quando o teste acaba.
      const { data: sup } = await admin
        .from("suppliers")
        .select("trial_ends_at")
        .eq("id", assinatura.supplier_id)
      ...
      const { data: jaFaturado } = await admin
        .from("subscription_invoices")
        .select("id")
  ...
  if (tipo === "destaque" && referenciaId) {
    ...
    if (!destaque) {
      await registrar({ resultado: "destaque_nao_encontrado" }, 200);
      return json({ ok: true, ignored: "destaque_nao_encontrado" });
    }
    if (aprovado) {
  ...
  if (tipo === "cancelamento" && referenciaId) {
    if (aprovado) {
      await admin.from("idle_date_reservations").update({ taxa_cancelamento_status: "paga" }).eq("id", referenciaId);
    }
    await registrar(
      { resultado: aprovado ? "cancelamento_pago" : "cancelamento_atualizado", reservation_id: referenciaId },
      200,
    );
    return json({ ok: true, tipo, ambiente, status });
  }
  ...
  if (!reserva) {
    const { data } = await admin
      ...
      .select("*")
      .eq("mp_split_payment_id", String(pagamento?.order?.id ?? paymentId))
      .maybeSingle();
    reserva = data;
  }
  if (!reserva) {
  ...
  await registrar(
    { resultado: aprovado ? "reserva_confirmada" : "reserva_atualizada", reservation_id: reserva.id },
    200,
  );
  return json({ ok: true, tipo, ambiente, status });
});
