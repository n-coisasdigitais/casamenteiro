...
export default function ReviewSupplierDialog({
  open,
  onOpenChange,
  jobId,
  supplierId,
  ...
  const salvar = async () => {
    if (!supplierId) {
      return toast({ title: "Não foi possível identificar o fornecedor", variant: "destructive" });
    }
    setLoading(true);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Avaliar {supplierName || "fornecedor"}</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          ...
          <Textarea
            rows={4}
            placeholder="Como foi trabalhar com este fornecedor? (opcional)"
            value={comentario}
