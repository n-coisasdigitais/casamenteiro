...
export default function AdminReviews() {
  ...
  const load = async () => {
    ...
    // Busca os nomes dos autores em uma query separada (sem quebrar o embed)
    ...
    setRows(reviews.map((r) => ({ ...r, autor_nome: nameMap[r.user_id] || "Casal" })));
  };
      ...
      <main className="container py-6">
        <div className="flex gap-2 mb-4">
          ...
          <Button size="sm" variant={filter === "low" ? "default" : "outline"} onClick={() => setFilter("low")}>
            Baixas (≤2★) ({rows.filter((r) => r.rating <= 2).length})
          </Button>
        </div>
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs text-muted-foreground">
                      por {r.autor_nome || "anônimo"} · {new Date(r.created_at).toLocaleDateString("pt-BR")}
                    </span>
                  </div>
                  {r.title && <p className="font-medium text-sm">{r.title}</p>}
                  <p className="text-sm text-muted-foreground">{r.comment}</p>
                </div>
                <Button variant="destructive" size="sm" onClick={() => removeReview(r.id)}>
